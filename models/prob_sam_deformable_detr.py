...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, losses, invalid_cls_logits, hidden_dim, focal_alpha=0.25, \
                empty_weight=0.1, temperature=1, geometirc_mean_alpha=0.5, iou_thre = 0.5, obj_thre=0.7, unmatched_boxes=True):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, num_pseudo_boxes, lvl, owod_targets, owod_indices):
        ...
        if len(pseudo_target_boxes) == 0:
            ...
            obj_weights = torch.exp(-self.temperature * pseudo_pred_obj).detach()
            ...
            pseudo_loss_giou = obj_weights * (1 - torch.diag(box_ops.generalized_box_iou(
    ...
    def loss_obj_likelihood(self, outputs, targets, indices, num_boxes, num_pseudo_boxes, lvl, owod_targets, owod_indices):
        ...
        src_boxes = outputs['pred_boxes'][idx]
        gt_target_boxes = torch.cat([t['boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0)
        iou = torch.diag(box_ops.box_iou(
            box_ops.box_cxcywh_to_xyxy(src_boxes),
            box_ops.box_cxcywh_to_xyxy(gt_target_boxes))[0]).detach()
        pred_obj = outputs["pred_obj"][idx]
        obj_prob = torch.exp(-self.temperature * pred_obj)
        loss_obj_ll = (- torch.log(obj_prob)).sum() / num_boxes

        pseudo_pred_obj = outputs["pred_obj"][owod_idx]
        pseudo_obj_prob = torch.exp(-self.temperature * pseudo_pred_obj)
        loss_pseudo_obj_ll = (- torch.log(pseudo_obj_prob)).sum() / num_pseudo_boxes

        return  {'loss_obj_ll': loss_obj_ll, 'loss_pseudo_obj_ll': loss_pseudo_obj_ll}
            ...
            for i in range(len(indices)):
                ...
                if len(segment_region) != 0:
                    with torch.no_grad():
                        ...
                        unmatched_objectnesses = eval_objectness[i][unmatched_indices]
                        unmatched_iou_cost = -box_ops.jaccard(unmatched_boxes, segment_region)# compute iou cost with segment_region
                        inverse_segment_region = box_ops.box_xyxy_to_cxcywh(segment_region / torch.tensor([img_w, img_h, img_w, img_h],dtype=torch.float32).to(owod_device))
                        cost_bbox = torch.cdist(owod_outputs['pred_boxes'][i][unmatched_indices, :], inverse_segment_region, p=1) # compute l1 cost with segment_region
                        unmatched_objectnesses_cost = -unmatched_objectnesses[:, None].repeat(1, len(segment_region)) # compute prob cost with segment_region
                        C = 5 * cost_bbox + 2 * unmatched_objectnesses_cost + 2 * unmatched_iou_cost
                        pred_unknown_indices, segment_unknown_indices = linear_sum_assignment(C.cpu())
                        # match to the first label assignment
                        pred_unknown_indices = unmatched_indices[pred_unknown_indices]
                        unknown_indices = (pred_unknown_indices, segment_unknown_indices)
                        
                        unk_label = torch.as_tensor([self.num_classes-1], device=owod_device)
                        owod_targets[i]['labels'] = unk_label.repeat_interleave(len(unknown_indices[0]))
                        owod_targets[i]['boxes'] = inverse_segment_region
                        owod_indices[i] = unknown_indices
                else:
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        print("prob:", torch.round(obj_prob, decimals=3))
...
def build(args):
    ...
    criterion = SetCriterion(num_classes, matcher, weight_dict, losses, invalid_cls_logits, args.hidden_dim, focal_alpha=args.focal_alpha, \
                            ...
                            iou_thre = args.iou_thre, obj_thre=args.obj_thre, unmatched_boxes=args.unmatched_boxes)
    criterion.to(device)
