...
def get_args_parser():
    ...
    # parser.add_argument('--lr', default=4e-4, type=float) # for 8 gpus
    ...
    parser.add_argument('--geometirc_mean_alpha', default=0.5, type=float)
    parser.add_argument('--post_geometirc_mean_alpha', default=1., type=float)
    parser.add_argument('--pseudo_obj_temp', default=1, type=float)
    parser.add_argument('--pseudo_obj_loss_coef', default=1, type=float)
    parser.add_argument('--pseudo_bbox_loss_coef', default=5, type=float)
    parser.add_argument('--pseudo_giou_loss_coef', default=2, type=float)
    parser.add_argument('--obj_thre', default=0.7, type=float)
    parser.add_argument('--iou_thre', default=0.4, type=float)
    parser.add_argument('--sns_thre', default=0.3, type=float)
    parser.add_argument('--lr_decline_p', default=0.5, type=float)
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            if (epoch + 1) % args.lr_drop == 0 or (epoch % args.eval_every == 0 or epoch == 0 or epoch == 1 or (args.epochs-epoch)<6):
                test_stats, coco_evaluator = evaluate(
                    model, criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir, args)
...
def get_datasets(args):
    ...
    dataset_train = OWDetection(args, args.data_root, ann='Annotations_segment', image_set=args.train_set, transforms=make_coco_transforms(args.train_set), dataset = args.dataset)
    dataset_val = OWDetection(args, args.data_root,ann='Annotations_segment', image_set=args.test_set, dataset = args.dataset, transforms=make_coco_transforms(args.test_set))
